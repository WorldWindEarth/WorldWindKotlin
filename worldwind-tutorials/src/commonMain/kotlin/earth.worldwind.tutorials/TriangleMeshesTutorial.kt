...
class TriangleMeshesTutorial(private val engine: WorldWind) : AbstractTutorial() {
    ...
    private val meshLayer = RenderableLayer("Circular Triangle Meshes")
    private val gridMeshLayer = RenderableLayer("Rectangular Triangle Mesh")
    ...
    private fun crearteMeshes() {
        ...
        highlightAttributes.interiorColor = Color(1f, 1f, 1f, 0.5f)
       ...
       val imageFactory = TriangleMeshImageFactory()
        ...
        meshAttributes = ShapeAttributes(meshAttributes)
        ...
        highlightAttributes = ShapeAttributes(highlightAttributes)
        highlightAttributes.interiorImageSource = ImageSource.fromImageFactory(imageFactory)
        ...
        val gridMeshAttributes = ShapeAttributes()
        gridMeshAttributes.outlineColor = Color(0f, 0f, 1f, 1f)
        gridMeshAttributes.interiorColor = Color(1f, 1f, 1f, 1f)
        gridMeshAttributes.interiorImageSource = ImageSource.fromResource(MR.images.splash_nww)
        gridMeshAttributes.isLightingEnabled = true
        ...
        gridMeshLayer.addRenderable(gridMesh);

        // The following is an example of 3D ray intersection with a TriangleMesh shape.
        // A ray will be generated extending from the camera "eye" point towards a point in the
        // TriangleMesh shape where the user has clicked, then the intersections between this
        // ray and the shape will be computed and displayed.
...
//            val meshes = listOf(firstMesh, secondMesh, gridMesh)
//
//            for (currentMesh in meshes) {
//                val intersections = currentMesh.rayIntersections(clickRay, wwd.renderContext)
//
