...
actual open class RenderResourceCache @JvmOverloads constructor(
    ...
    protected open fun cacheView(imageSource: ImageSource): ImageSource {
        ...
        return ImageSource.fromImageFactory(object : ImageSource.ImageFactory {
            override suspend fun createBitmap() = factory.createCachedBitmap()
    ...
    protected open fun retrieve(
        ...
        mainScope.launch {
            var cacheMiss = false
            try {
                val bitmap = imageDecoder.decodeImage(decodeSource, options)
                when {
                    bitmap != null && !bitmap.isRecycled -> retrievalSucceeded(imageSource, options, bitmap)
