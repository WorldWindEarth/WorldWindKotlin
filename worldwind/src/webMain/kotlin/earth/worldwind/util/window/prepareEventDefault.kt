...
val prepareEventDefault: PrepareEventHandler = { dirtyEvent ->
    when (true) {
        ...
        dirtyEvent.type.startsWith("touch") -> try {
            ...
            cloneTouchEvent(event)
        } catch (e: Throwable) {
            jsConsole.warn("Failed to adapt touch event: ${dirtyEvent.type}. May window will fail even type checks.\n$e")
            dirtyEvent
        }

        else -> {
            jsConsole.info("Event converter between windows is not implemented for event type: ${dirtyEvent.type}.")
            dirtyEvent
        }
    }
}

// Top-level js() helper (Kotlin/Wasm requires js() bodies at file scope; `event` binds by name).
// `TouchEvent`'s constructor can't take the event type, so re-create it from the source touch event.
private fun cloneTouchEvent(event: TouchEvent): TouchEvent = js(
...
// Kotlin/Wasm has no `kotlin.js.console`; bind the global console as a typed external instead.
