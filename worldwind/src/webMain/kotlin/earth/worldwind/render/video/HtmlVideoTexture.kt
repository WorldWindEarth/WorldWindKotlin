...
/**
 * Web (js + wasmJs) [earth.worldwind.render.Texture] that mirrors a playing `HTMLVideoElement` onto a
...
class HtmlVideoTexture(
    ...
    override fun bindTexture(dc: DrawContext): Boolean {
        ...
        val kgl = dc.gl as? WebKgl ?: return true
        // Upload the current decoded frame straight from the HTMLVideoElement via the WebGL
        // DOM-source texImage2D overload ([WebKgl.texImage2D]) — the browser decodes and
        // color-converts, no CPU pixel copy. HTMLVideoElement is itself a TexImageSource.
        kgl.texImage2D(GL_TEXTURE_2D, 0, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE, video)
        return true
    }
    ...
    private fun installFrameRedrawTrigger() {
        ...
        if (jsTypeof(v.requestVideoFrameCallback) == "function") {
...
@Suppress("UNUSED_PARAMETER") private fun jsTypeof(value: JsAny?): String = js("typeof value")
...
private external interface RequestVideoFrameCallbackHost : JsAny {
    ...
    fun requestVideoFrameCallback(callback: (Double, JsAny) -> Unit): Int
}
