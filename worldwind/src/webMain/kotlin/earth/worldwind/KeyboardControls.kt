...
open class KeyboardControls(protected val wwd: WorldWindow) {
    ...
    protected open fun startHold(keyCode: Int) {
        ...
        heldKeys[keyCode] = 0
        applyHeldKeys()
        // Kotlin/Wasm setInterval wants a () -> JsAny? callback; return null to satisfy it.
        if (tickIntervalId == -1) tickIntervalId = window.setInterval({ tick(); null }, TICK_INTERVAL_MS)
    }
