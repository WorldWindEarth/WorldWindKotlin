...
actual open class TiledElevationCoverage actual constructor(
    ...
    actual override suspend fun retrieveTileArray(key: Long, tileMatrix: TileMatrix, row: Int, column: Int) {
        ...
        if (elevationSource.isUrl) {
            ...
            try {
                ...
                val response = window.fetch(url, jso()).await()
                if (response.ok) {
                    ...
                    val pixels = when {
                        ...
                        contentType.equals("application/bil16", true) -> Int16ArrayToShortArray(int16ArrayOf(arrayBuffer))
                        contentType.equals("application/bil32", true) -> Float32ArrayToShortArray(float32ArrayOf(arrayBuffer))
    ...
    private fun arrayBufferToBytes(arrayBuffer: ArrayBuffer): ByteArray {
        ...
        val view = int8ArrayOf(arrayBuffer)
...
// Top-level js() helper (Kotlin/Wasm requires js() bodies at file scope; param binds by name).
@Suppress("UNUSED_PARAMETER") private fun decodeArrayBufferToText(arrayBuffer: ArrayBuffer): String = js("new TextDecoder().decode(arrayBuffer)")

// Typed-array views over an ArrayBuffer. Top-level js() helpers (Kotlin/Wasm requires js()
// bodies at file scope; params bind by name).
@Suppress("UNUSED_PARAMETER") private fun int16ArrayOf(buffer: ArrayBuffer): Int16Array = js("new Int16Array(buffer)")
@Suppress("UNUSED_PARAMETER") private fun float32ArrayOf(buffer: ArrayBuffer): Float32Array = js("new Float32Array(buffer)")
@Suppress("UNUSED_PARAMETER") private fun int8ArrayOf(buffer: ArrayBuffer): Int8Array = js("new Int8Array(buffer)")
