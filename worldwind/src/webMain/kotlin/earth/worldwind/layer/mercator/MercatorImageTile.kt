...
actual open class MercatorImageTile actual constructor(
    ...
    companion object {
        ...
        private val srcCtx = srcCanvas.getContext("2d", willReadFrequentlyOptions()) as CanvasRenderingContext2D
    ...
    override suspend fun <Resource> process(resource: Resource): Resource = when (resource) {
        ...
        is Image -> {
            ...
            dstCanvas.toBlob({
                ...
                else resource.onerror?.invoke("Error saving canvas to Blob".toJsString(), "", 0, 0, null)
            })
...
// Top-level js() factory (Kotlin/Wasm requires js() bodies at file scope) for the 2D context options bag.
private fun willReadFrequentlyOptions(): JsAny? = js("({ willReadFrequently: true })")