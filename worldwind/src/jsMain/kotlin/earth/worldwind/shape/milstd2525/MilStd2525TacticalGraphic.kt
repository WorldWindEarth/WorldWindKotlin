        ...
        when (si.getShapeType()) {
            ShapeInfo.SHAPE_TYPE_POLYLINE, ShapeInfo.SHAPE_TYPE_FILL -> {
                val shapeAttributes = ShapeAttributes().apply {
                    ...
                    outlineColor = convertColor(si.getLineColor())
                    interiorColor = convertColor(si.getFillColor() ?: si.getLineColor())
                }
                ...
                val outlineAttributes = if (hasOutline) ShapeAttributes(shapeAttributes).apply {
                    outlineColor = convertColor(RendererUtilities.getIdealOutlineColor(si.getLineColor()).apply { setAlpha(0.5f) })
