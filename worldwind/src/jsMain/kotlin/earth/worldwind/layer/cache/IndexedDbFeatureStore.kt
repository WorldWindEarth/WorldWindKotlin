...
internal class IndexedDbFeatureStore(
    ...
    override suspend fun readAll(): Flow<CachedFeatureRow> = flow {
        ...
        val req = store.index(INDEX_BY_CONTENT).openCursor(contentKey)
        val rows = mutableListOf<CachedFeatureRow>()
        idbWalkCursor(req) { cursor ->
            val record = cursor.value
            if (record.z == null) {
                val geom = record.geometry as? String
                val props = record.properties as? String
    ...
    override suspend fun replaceAll(rows: Flow<CachedFeatureRow>) {
        ...
        val store = tx.objectStore(FEATURES_STORE)
        val cursorReq = store.index(INDEX_BY_CONTENT).openCursor(contentKey)
        idbWalkCursor(cursorReq) { cursor ->
            if (cursor.value.z == null) cursor.delete()
        }
    ...
    override suspend fun readTile(z: Int, x: Int, y: Int): Flow<CachedFeatureRow>? {
        val tx = db.transaction(FEATURES_STORE, "readonly")
        val store = tx.objectStore(FEATURES_STORE)
        val req = store.index(INDEX_BY_TILE).getAll(arrayOf<Any>(contentKey, z, x, y))
        val arr = idbAwait<Array<dynamic>>(req)
        ...
        return arr.asSequence()
            .mapNotNull { record ->
                val geom = (record.geometry as? String)?.let { decodeGeometry(it) } ?: return@mapNotNull null
                CachedFeatureRow(geom, record.properties as? String)
            }
            .toList()
            .asFlow()
    }
    ...
    override suspend fun writeTile(z: Int, x: Int, y: Int, rows: Flow<CachedFeatureRow>) {
        ...
        val cursorReq = store.index(INDEX_BY_TILE).openCursor(arrayOf<Any>(contentKey, z, x, y))
    ...
    override suspend fun sizeBytes(): Long {
        ...
        var total = 0L
        idbWalkCursor(req) { cursor ->
            val geom = cursor.value.geometry as? String
            val props = cursor.value.properties as? String
