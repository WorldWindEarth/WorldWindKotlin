...
class WebContentManager(
    ...
    private suspend fun openDatabase(): IDBDatabase = suspendCancellableCoroutine { cont ->
        ...
        request.onupgradeneeded = { _: Event ->
            ...
            if (!db.objectStoreNames.contains(IndexedDbFeatureStore.FEATURES_STORE)) {
                ...
                val store = db.createObjectStore(IndexedDbFeatureStore.FEATURES_STORE, params)
                store.createIndex(
                    IndexedDbFeatureStore.INDEX_BY_TILE,
                    arrayOf("contentKey", "z", "x", "y"),
                    js("({unique: false})"),
                )
                store.createIndex(
    ...
    private suspend fun readIsFloat(contentKey: String): Boolean? = idbSerializationLock.withLock {
        runCatching {
            ...
            val req = store.get(contentKey)
            suspendCancellableCoroutine<Boolean?> { cont ->
                req.onsuccess = { _: Event ->
                    val raw = req.result
                    cont.resume(if (raw == undefined) null else raw.unsafeCast<Boolean>())
                }
                req.onerror = { _: Event ->
                    cont.resume(null)
                }
    ...
    private suspend fun rememberIsFloat(contentKey: String, isFloat: Boolean): Unit = idbSerializationLock.withLock {
        runCatching {
            ...
            val store = tx.objectStore(IS_FLOAT_STORE)
            val getReq = store.get(contentKey)
            suspendCancellableCoroutine<Unit> { cont ->
                getReq.onsuccess = { _: Event ->
                    if (getReq.result == undefined) {
                        val putReq = store.put(isFloat, contentKey)
    ...
    private suspend fun rememberDataType(contentKey: String, dataType: CacheEntry.DataType) {
        try {
            val db = db()
    ...
    private suspend fun rememberDisplayName(contentKey: String, displayName: String) {
        try {
            val db = db()
            val tx = db.transaction(DISPLAY_NAME_STORE, "readwrite")
            val store = tx.objectStore(DISPLAY_NAME_STORE)
            val existing = idbAwait<String?>(store.get(contentKey))
    ...
    override suspend fun registerWebService(contentKey: String, info: WebServiceInfo) {
        try {
            val db = db()
    ...
    private suspend fun loadWebServiceInfo(contentKey: String): WebServiceInfo? {
        ...
        val tx = db.transaction(METADATA_STORE, "readonly")
        val store = tx.objectStore(METADATA_STORE)
        val record = idbAwait<IdbWebServiceRecord?>(store.get(contentKey)) ?: return null
    ...
    override suspend fun listEntries(): List<CacheEntry> {
        ...
        run {
            ...
            val req = store.openCursor()
            idbWalkCursor(req) { cursor ->
                val key = cursor.key as? String ?: return@idbWalkCursor
                val record = cursor.value.unsafeCast<IdbWebServiceRecord?>() ?: return@idbWalkCursor
    ...
    override suspend fun setDisplayName(contentKey: String, displayName: String) {
        try {
            val db = db()
            val tx = db.transaction(DISPLAY_NAME_STORE, "readwrite")
    ...
    override suspend fun clearEntry(contentKey: String) {
        try {
            val db = db()
    ...
    override suspend fun deleteEntry(contentKey: String) {
        val db = db()
        for (storeName in TILE_STORES) {
            val tx = db.transaction(storeName, "readwrite")
            val store = tx.objectStore(storeName)
        ...
        metaTx.objectStore(METADATA_STORE).delete(contentKey)
        ...
        dtTx.objectStore(DATA_TYPE_STORE).delete(contentKey)
        idbAwaitTransaction(dtTx)
        // Drop the user-visible name.
        runCatching {
            val nameTx = db.transaction(DISPLAY_NAME_STORE, "readwrite")
            nameTx.objectStore(DISPLAY_NAME_STORE).delete(contentKey)
 ...
 */
private external interface IdbWebServiceRecord {
    var type: String
    var address: String
...
private fun newIdbWebServiceRecord(
    ...
    val r = js("({})").unsafeCast<IdbWebServiceRecord>()
    r.type = type
    r.address = address
