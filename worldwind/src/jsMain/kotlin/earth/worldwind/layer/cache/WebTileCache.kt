...
internal object WebTileCache {
    ...
    suspend fun fetchWithCache(url: String, storeName: String): Response {
        ...
        if (response.ok) {
            val stamped = stampResponse(response.asDynamic().clone())
    ...
    suspend fun evictByPolicy(storeName: String, policy: CacheEvictionPolicy) {
        ...
        for (req in toDelete) cache.asDynamic().delete(req).unsafeCast<Promise<Boolean>>().await()
    }
     ...
     */
    private fun stampResponse(original: dynamic): dynamic {
        val now = js("Date.now()")
        return js(
