    ...
    actual fun setSymbolModifier(
        ...
        if (result.length >= 11 && !SymbolUtilities.isTacticalGraphic(result)) {
            ...
            if (mobility != null && mobility.length == 2 && result.length >= 12
                ...
                val modifier = if (hq && SymbolUtilities.canUnitHaveModifier(
                        result,
                        ModifiersUnits.S_HQ_STAFF_OR_OFFSET_INDICATOR
                    )
                ) "A" // Headquarters
                    ...
                    && feintDummy && SymbolUtilities.canUnitHaveModifier(
                        result,
                        ModifiersUnits.AB_FEINT_DUMMY_INDICATOR
                    )
                ) "D" // TaskForce/Feint/Dummy
                else if (taskForce && SymbolUtilities.canUnitHaveModifier(
                        result,
                        ModifiersUnits.D_TASK_FORCE_INDICATOR
                    )
                ) "B" // TaskForce
                else if (feintDummy && SymbolUtilities.canUnitHaveModifier(
                        result,
                        ModifiersUnits.AB_FEINT_DUMMY_INDICATOR
                    )
                ) "C" // Feint/Dummy
