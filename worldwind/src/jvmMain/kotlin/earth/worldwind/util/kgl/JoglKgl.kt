...
class JoglKgl(private val gl: GL3ES3) : Kgl {
    ...
    override fun bufferData(target: Int, size: Int, sourceData: ShortArray?, usage: Int, offset: Int) =
        gl.glBufferData(target, size.toLong(), sourceData?.let { ShortBuffer.wrap(sourceData, offset, sourceData.size - offset)}, usage)

    override fun bufferData(target: Int, size: Int, sourceData: IntArray?, usage: Int, offset: Int) =
        gl.glBufferData(target, size.toLong(), sourceData?.let { IntBuffer.wrap(sourceData, offset, sourceData.size - offset)}, usage)

    override fun bufferData(target: Int, size: Int, sourceData: FloatArray?, usage: Int, offset: Int) =
        gl.glBufferData(target, size.toLong(), sourceData?.let { FloatBuffer.wrap(sourceData, offset, sourceData.size - offset)}, usage)
