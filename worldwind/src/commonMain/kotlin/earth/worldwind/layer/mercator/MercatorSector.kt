...
open class MercatorSector(
    ...
    private fun getY(latitude: Angle) =
        (SlippyTiles.latRadiansToMercatorY(latitude.inRadians) - minLatPercent) / (maxLatPercent - minLatPercent)
    ...
    companion object {
        ...
        @JvmStatic
        fun gudermannianInverse(latitude: Angle) = SlippyTiles.latRadiansToMercatorY(latitude.inRadians)

        @JvmStatic
        fun gudermannian(percent: Double) = fromRadians(SlippyTiles.mercatorYToLatRadians(percent))
    }
}