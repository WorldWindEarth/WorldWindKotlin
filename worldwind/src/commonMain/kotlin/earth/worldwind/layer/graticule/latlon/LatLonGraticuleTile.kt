...
internal class LatLonGraticuleTile(
    ...
    private var subTiles: List<LatLonGraticuleTile>? = null
    ...
    private fun createSubTiles(): List<LatLonGraticuleTile> {
        val formatFits = layer.angleFormat == AngleFormat.DMS || layer.angleFormat == AngleFormat.DM
        val subDivisions = if (formatFits && (level == 0 || level == 2)) 6 else 10
        return subdivide(divisions).map { LatLonGraticuleTile(layer, it, subDivisions, level + 1) }
    }
