...
open class AtmosphereLayer: AbstractLayer("Atmosphere") {
    ...
    private fun renderBrunetonGround(rc: RenderContext) {
        ...
        // Same convention as the legacy path: night image only when the engine carries a
        // scene time (the sun position is then independent of the camera, so a terminator
        // is meaningful).
        drawable.nightTexture = rc.time?.run { rc.getTexture(nightImageSource, nightImageOptions) }
        rc.offerSurfaceDrawable(drawable, Double.POSITIVE_INFINITY)
    }
