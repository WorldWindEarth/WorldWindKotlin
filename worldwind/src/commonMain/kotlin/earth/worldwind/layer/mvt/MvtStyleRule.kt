...
class MvtRuleBasedStyle(
    ...
    // Rules grouped by exact source layer (built once) — per-feature matching scans only the layer's
    // bucket instead of the full list, skipping the doomed layer-mismatch compares. groupBy keeps order,
    // so first-match is identical to scanning all rules.
    private val rulesByLayer = rules.groupBy { it.sourceLayer }
    ...
    fun firstShapeAndText(
        ...
        out.shape = null
        out.text = null
        val bucket = rulesByLayer[layerName] ?: return
        var needShape = true
        var needText = true
    ...
    fun firstMatching(
        ...
        val bucket = rulesByLayer[layerName] ?: return null
        for (i in bucket.indices) {
            val r = bucket[i]
