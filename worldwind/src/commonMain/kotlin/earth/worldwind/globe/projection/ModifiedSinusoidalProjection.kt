...
open class ModifiedSinusoidalProjection : Abstract2DProjection() {
    ...
    override fun geographicToCartesianGrid(
        ...
        require(numLat >= 1 && numLon >= 1) {
            logMessage(
                ERROR, "ModifiedSinusoidalProjection", "geographicToCartesianGrid",
                "Number of latitude or longitude locations is less than one"
            )
        }
        require(height == null || height.size >= numLat * numLon) {
            logMessage(ERROR, "ModifiedSinusoidalProjection", "geographicToCartesianGrid", "missingArray")
        }
        ...
        for (latIndex in 0 until numLat) {
            ...
            val cosLat = cos(lat)
            val powLat = if (cosLat > 0) cosLat.pow(EXPONENT) else 0.0

            var lon = minLon
            var colIndex = rowIndex
            for (lonIndex in 0 until numLon) {
                if (lonIndex == numLon - 1) lon = maxLon
                result[colIndex++] = (eqr * lon * powLat - xOffset).toFloat()
    ...
    override fun geographicToCartesianBorder(
        ...
        for (latIndex in 0 until numLat) {
            ...
            while (lonIndex < numLon) {
                ...
                result[resultIndex++] = (eqr * lon * powLat - xOffset).toFloat()
