...
open class GeographicMesh(
    ...
    var positions: Array<Array<Position>> = positions
        set(value) {
            ...
            field = value
            referencePosition = determineReferencePosition(value)
            reset()
        }
    ...
    init {
        ...
        referencePosition = determineReferencePosition(positions)
    }
    ...
    override fun computeMeshPoints(rc: RenderContext): FloatArray {
        ...
        var eyeDistSquared = Double.MAX_VALUE
        var k = 0
        val eyePoint = rc.cameraPoint
        val meshPoints = FloatArray(numRows * numColumns * 3)
        val pt = Vec3()
        ...
        for (r in positions.indices) {
            for (c in positions[r].indices) {
                ...
                val dSquared = pt.distanceToSquared(eyePoint)
                if (dSquared < eyeDistSquared) eyeDistSquared = dSquared
        ...
        currentData.cameraDistance = sqrt(eyeDistSquared)
    ...
    override fun computeOutlineIndices(): ShortArray {
        ...
        // Top row
        for (c in 0 until numColumns) {
            outlineIndices[k++] = c.toShort()
        }

        // Right column
        for (r in 1 until numRows) {
            outlineIndices[k++] = ((r + 1) * numColumns - 1).toShort()
        }

        // Bottom row (reversed)
        for (c in numRows * numColumns - 2 downTo (numRows - 1) * numColumns) {
            outlineIndices[k++] = c.toShort()
        }

        // Left column (reversed)
        for (r in numRows - 2 downTo 0) {
            outlineIndices[k++] = (r * numColumns).toShort()
        }
     ...
     * @param rc The render context containing the globe
     * @returns List of Intersection objects sorted by distance from ray origin
     */
    override fun rayIntersections(ray: Line, rc: RenderContext): Array<Intersection> {
        val globe = rc.globe

        // Ensure we have current mesh points
        if (currentData.refreshVertexArray) currentData.vertexArray = computeMeshPoints(rc)
