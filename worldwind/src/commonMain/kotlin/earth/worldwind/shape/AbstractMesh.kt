...
abstract class AbstractMesh(attributes: ShapeAttributes) : AbstractShape(attributes) {
     ...
     **/
    final override var referencePosition = Position()
    ...
    open fun createSurfaceShape(): AbstractShape? = null
    ...
    override fun moveTo(globe: Globe, position: Position) { referencePosition.copy(position) }
    ...
    override fun makeDrawable(rc: RenderContext) {
        ...
        rc.offerShapeDrawable(drawable, currentData.cameraDistance)
    }
    ...
    protected open fun drawInterior(rc: RenderContext, drawState: DrawShapeState) {
        ...
        if (imageSource != null && (!rc.isPickMode || !pickTransparentImagePixels)) {
            rc.getTexture(imageSource)?.let { texture ->
                ...
                //drawState.texCoordMatrix.multiplyByMatrix(texture.coordTransform)
            }
        }
    ...
    protected open fun drawOutline(rc: RenderContext, drawState: DrawShapeState) {
        ...
        drawState.drawElements(GL_LINE_STRIP, currentData.meshOutlineIndices.size, GL_UNSIGNED_SHORT, currentData.meshIndices.size)
    }
    ...
    protected open fun assembleGeometry(rc: RenderContext) {
        ...
        with(currentBoundindData) {
            boundingBox.setToPoints(currentData.vertexArray, currentData.vertexArray.size, stride = 3)
    ...
    protected open fun computeNormals(): FloatArray {
        ...
        for (i in normals.indices) {
            normals[i]?.let { normalI ->
                ...
                normal.normalize()
                normalsBuffer[i * 3] = normal.x.toFloat()
                normalsBuffer[i * 3 + 1] = normal.y.toFloat()
                normalsBuffer[i * 3 + 2] = normal.z.toFloat()
            } ?: run {
                normalsBuffer[i * 3] = 0f
                normalsBuffer[i * 3 + 1] = 0f
                normalsBuffer[i * 3 + 2] = 0f
            }
        }
