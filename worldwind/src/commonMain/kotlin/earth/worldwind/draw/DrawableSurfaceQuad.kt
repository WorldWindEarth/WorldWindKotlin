...
open class DrawableSurfaceQuad protected constructor() : Drawable {
        ...
        try {
            ...
            for (idx in scratchList.indices) {
                ...
                if (shape.drawState.isLine) {
                    if (!drawShapesToTextureOutline(
                            dc, terrain, shape,
                            colorAttachment.width.toFloat(), colorAttachment.height.toFloat()
                        )
                    ) continue
                } else {
                    if (!drawShapesToTextureInterior(dc, terrain, shape)) continue
                }
            }
    ...
    protected open fun drawShapesToTextureOutline(
        dc: DrawContext, terrain: DrawableTerrain, shape: DrawableSurfaceQuad,
        colorAttachmentWidth: Float, colorAttachmentHeight: Float
    ): Boolean {
