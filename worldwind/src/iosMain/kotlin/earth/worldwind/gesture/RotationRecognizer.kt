...
open class RotationRecognizer : GestureRecognizer {
    ...
    protected open fun shouldRecognize(event: TouchEvent): Boolean {
        ...
        val angle = currentTouchAngle(event)
        val rot = normalizeAngle180((angle - referenceAngle).toDouble()).toFloat()
        return abs(rot) > interpretAngle
    }

    protected open fun currentTouchAngle(event: TouchEvent): Float {
        val index0 = event.findPointerIndex(pointerIds[0])
        val index1 = event.findPointerIndex(pointerIds[1])
        if (index0 < 0 || index1 < 0) return referenceAngle
